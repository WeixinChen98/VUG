    ...
    def export_result(self, output_file=None):
        ...
        with open(output_file, 'w') as fp:
            for params in self.params2result:
                fp.write(params + '\n')
                valid_result_str = ''
                for key, value in self.params2result[params]['best_valid_result'].items():
                    valid_result_str +=  f'\n{key}:\n{dict2str(value)}'
                fp.write('Valid result:' + valid_result_str + '\n')
                
                test_result_str = ''
                for key, value in self.params2result[params]['test_result'].items():
                    test_result_str +=  f'\n{key}:\n{dict2str(value)}'
                fp.write('Test result:' + test_result_str + '\n\n')
